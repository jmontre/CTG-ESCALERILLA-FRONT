...
export default function EditPlayerModal({ isOpen, onClose, onSuccess, player, allPlayers = [] }: EditPlayerModalProps) {
  ...
  useEffect(() => {
    if (player) {
      setFormData({
        ...
        immune_until:            player.immune_until     ? new Date(player.immune_until).toISOString().slice(0,16)     : '',
        vulnerable_until:        player.vulnerable_until ? new Date(player.vulnerable_until).toISOString().slice(0,16) : '',
        member_type:             (player as any).member_type  || 'socio',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      updateData.position         = formData.position !== '' ? parseInt(String(formData.position)) : null;
      updateData.immune_until     = formData.immune_until     ? new Date(formData.immune_until).toISOString()     : null;
      updateData.vulnerable_until = formData.vulnerable_until ? new Date(formData.vulnerable_until).toISOString() : null;
  ...
  const availableParents = allPlayers.filter(p => p.id !== player.id && (p as any).member_type !== 'hijo_socio');
      ...
      <div className="relative w-full max-w-2xl animate-scale-in">
        <div className="bg-[#0f2211] border border-ctg-green/15 rounded-2xl shadow-2xl shadow-black/60 overflow-hidden max-h-[85vh] overflow-y-auto">
          <div className="bg-[#152b18] border-b border-[#1e4020] px-6 py-4 flex items-center justify-between sticky top-0 z-10">
            <h3 className="font-display text-xl font-bold text-[#F0F7E8]">Editar Jugador</h3>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            {/* ── 2. Escalerilla ── */}
            <div className={sectionClass}>
              <h3 className={sectionTitle}>🎾 Escalerilla</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                  <label className={labelClass}>Posición</label>
                  <input type="number" min="1" value={formData.position} placeholder="Sin posición"
                    onChange={e => setFormData({...formData, position: e.target.value})} className={inputClass} />
                  <p className="text-xs text-[#F0F7E8]/40 mt-1">Vacío = no compite</p>
                </div>
                <div>
                  <label className={labelClass}>Victorias</label>
                  <input type="number" min="0" value={formData.wins}
                    onChange={e => setFormData({...formData, wins: parseInt(e.target.value)||0})} className={inputClass} />
                </div>
                <div>
                  <label className={labelClass}>Derrotas</label>
                  <input type="number" min="0" value={formData.losses}
                    onChange={e => setFormData({...formData, losses: parseInt(e.target.value)||0})} className={inputClass} />
                </div>
              </div>
              <p className="text-xs text-[#F0F7E8]/45 mb-4">Total: {formData.wins + formData.losses} partidos</p>

              <div className="mb-3">
                <label className={labelClass}>🛡️ Inmunidad (hasta)</label>
                <div className="flex gap-2">
                  <input type="datetime-local" value={formData.immune_until}
                    onChange={e => setFormData({...formData, immune_until: e.target.value})} className={`flex-1 ${inputClass}`} />
                  <button type="button" onClick={() => { const d = new Date(); d.setHours(d.getHours()+24); setFormData({...formData, immune_until: d.toISOString().slice(0,16)}); }}
                    className="px-3 py-2 bg-blue-900/30 border border-blue-500/20 text-blue-300 rounded-lg text-xs hover:bg-blue-900/50 transition">+24h</button>
                  <button type="button" onClick={() => setFormData({...formData, immune_until: ''})}
                    className="px-3 py-2 bg-[#1e4020] text-[#F0F7E8]/60 rounded-lg text-xs hover:bg-[#26502a] transition">Quitar</button>
                </div>
              </div>
              <div>
                <label className={labelClass}>⚠️ Vulnerabilidad (hasta)</label>
                <div className="flex gap-2">
                  <input type="datetime-local" value={formData.vulnerable_until}
                    onChange={e => setFormData({...formData, vulnerable_until: e.target.value})} className={`flex-1 ${inputClass}`} />
                  <button type="button" onClick={() => { const d = new Date(); d.setHours(d.getHours()+24); setFormData({...formData, vulnerable_until: d.toISOString().slice(0,16)}); }}
                    className="px-3 py-2 bg-orange-900/30 border border-orange-500/20 text-orange-300 rounded-lg text-xs hover:bg-orange-900/50 transition">+24h</button>
                  <button type="button" onClick={() => setFormData({...formData, vulnerable_until: ''})}
                    className="px-3 py-2 bg-[#1e4020] text-[#F0F7E8]/60 rounded-lg text-xs hover:bg-[#26502a] transition">Quitar</button>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <select value={formData.member_type}
                    onChange={e => setFormData({...formData, member_type: e.target.value, parent_id: ''})} className={`${inputClass} select`}>
                ...
                {formData.member_type === 'hijo_socio' && (
                  <div>
                    ...
                    <select value={formData.parent_id}
                      onChange={e => setFormData({...formData, parent_id: e.target.value})} className={`${inputClass} select`}>
                ...
                {formData.member_type === 'socio' && (
                  <div>
                    <label className={labelClass}>Cupos extra alta demanda (semana)</label>
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => setFormData({...formData, extra_high_demand_slots: Math.max(0, formData.extra_high_demand_slots - 1)})}
                      ...
                      <button type="button" onClick={() => setFormData({...formData, extra_high_demand_slots: formData.extra_high_demand_slots + 1})}
                    ...
                    <p className="text-xs text-[#F0F7E8]/40 mt-1">
                      Total esta semana: {2 + (formData.extra_high_demand_slots || 0)} turnos
                    </p>
                  </div>
                )}
                ...
                <div className="md:col-span-2">
                  <label className="flex items-center gap-3 cursor-pointer p-3 rounded-lg border border-[#1e4020] hover:bg-[#0f2211] transition">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#F0F7E8]/40">No podrá hacer nuevas reservas</p>
                    </div>
                  </label>
                </div>
              </div>

              {/* Reservas activas del jugador */}
              {loadingReservations ? (
                ...
                <>
                  ...
                  {pastReservations.length > 0 && (
                    <>
                      <p className="text-xs font-semibold text-[#F0F7E8]/35 uppercase tracking-wide mb-2">Últimas canceladas/pasadas</p>
            ...
            <div className={sectionClass}>
              ...
              <select value={formData.admin_role}
                onChange={e => setFormData({...formData, admin_role: e.target.value})} className={`${inputClass} select`}>
              ...
              <p className="text-xs text-[#F0F7E8]/40 mt-1">
                {!formData.admin_role            && 'Jugador normal, sin acceso a paneles admin'}
